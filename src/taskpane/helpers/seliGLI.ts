...
export async function populateSeliGLI(): Promise<{
  ...
  const augmentedIndicators: SeliIndicatorExtended[] = indicators.map((ind) => {
    if (ind.forOutcome && outcomeThemeMap[ind.forOutcome] && !(ind as any).forTheme) {
      return { ...ind, forTheme: outcomeThemeMap[ind.forOutcome] };
    }
    return ind as SeliIndicatorExtended;
  });
  ...
  await Excel.run(async (context) => {
    ...
    await upsertEntity(context, "Outcome", outcomes, {
      simple: ["@id", "hasName", "hasDescription"],
      links: ["forTheme", "hasIndicator"],
    });
    await upsertEntity(context, "Indicator", augmentedIndicators, {
      simple: ["@id", "hasName", "hasDescription"],
      links: ["forOutcome", "forTheme"],
    });
    try {
